...
shipped in `skillmind/src/` and is kept only for reference. See
...
by the shipped app**: the DBMS concept graph actually used (`skillmind/src/lib/dbmsGraph.ts`) is
hand-written directly in code, not extracted from `data/dbms.md` or any other file here.

## `AgentSpec-SkillMind-1.md`
...
`skillmind/src/lib/backup.ts` (185 lines) — the pre-written backup question sets used when live
...
**`skillmind/src/lib/demoData.ts` and `skillmind/src/lib/demoSub.ts` (167 lines combined) are a
genuine exception to "authored during this build."** `backup.ts` imports both and its own code
comment labels them "the earlier hand-written bank" ([backup.ts:128](skillmind/src/lib/backup.ts#L128)),
...
Listed in `skillmind/package.json` but unreferenced anywhere in `skillmind/src/` (grepped,
...
`slice/`, `web/`, `scripts/` (except `skillmind.py`, a stub), `tests/`, `docs/`, `corpus/`, `.devcontainer/`,
...
submitted for judging is `skillmind/`.
